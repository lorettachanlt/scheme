...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        while count < len_formals:
            frame.bindings[formals[count]] = vals[count]
            count += 1
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            lambda_form = Pair(target.second, vals.second)
            env.bindings[target.first] = do_lambda_form(lambda_form, env)
            return target.first
        else:
...
def do_let_form(vals, env):
    ...
    while count < len(elements):
        names = Pair(elements[count][0], names)
        evaluated = scheme_eval(elements[count][1], env)
        values = Pair(evaluated, values)
        count += 1
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    evaluated = scheme_eval(vals[0], env)
    if len(vals) == 3:
...
def do_and_form(vals, env):
    ...
    exp = read_line(str(vals))
    count = 0

    if len(exp) == 0:
        return True
    while count < (len(exp) - 1):
        if scheme_true(scheme_eval(exp[count], env)) == True:
            count += 1
        else:
            return False
    if count == (len(exp) - 1) :
        return scheme_eval(exp[len(exp) - 1], env)
...
def do_or_form(vals, env):
    ...
    while count < (len(exp) - 1):
        if scheme_true(scheme_eval(exp[count], env)) == True:
            return quote(scheme_eval(exp[count], env))
        else:
            count += 1
    if count == (len(exp) - 1):
        return scheme_eval(exp[len(exp) - 1], env)
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            make_begin = Pair("begin", Pair(test, clause.second))
            return make_begin
    return okay
...
def check_formals(formals):
    ...
    x = formals
    while x != nil:
        if scheme_symbolp(x.first) == False:
            raise SchemeError
        x = x.second
